document.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementsByClassName("crazy")[0].addEventListener('click',startcrazy);
	function startcrazy() {
		toggleButtons.forEach((button)=> {
			button.click();
		})
		document.getElementsByClassName("crazy")[0].innerHTML = `전체 ${toggleButtons[0].innerHTML}`;
	}
	document.getElementsByClassName("sudoinit")[0].addEventListener('click',sudoinit);
	function sudoinit() {
		resetButtons.forEach((button)=> {
			button.click();
		})
